...
impl fmt::Display for LexError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    match *self {
      ...
      IllegalCharacter(c) =>
        write!(f, "\"{}\" is not allowed in source", c),
      InvalidNumber =>
      ...
      NonHexCharInHex =>
        write!(f, "non-hexadecimal (0-9 + a-f) character in hexadecimal number"),
    }
  }
}
...
impl fmt::Display for ParseError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    match *self {
      MismatchedLParen =>
        write!(f, "mismatched \"(\" left parenthesis"),
      MismatchedRParen =>
        write!(f, "mismatched \")\" right parenthesis"),
      QuotedRParen =>
...
impl fmt::Display for EvalError {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    ...
    match *self {
      ...
      InvalidLambdaName =>
        write!(f, "lambda name must be a symbol"),
      InvalidParameter =>
        write!(f, "parameters must be symbols"),
      InvalidParameterList =>
      ...
      IOError(ref e) =>
        write!(f, "{}", e),
      NonBooleanTest =>
        write!(f, "non-boolean expression in conditional"),
      NonFunctionApplication =>
        write!(f, "non-function values cannot be applied"),
      TooManyArguments =>
        write!(f, "too many arguments given"),
      UnboundSymbol =>
