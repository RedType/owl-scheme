        ...
        if dotted {
          if let Some(LexItem(lexeme, info)) = lexemes.peek() {
            if *lexeme != Lexeme::RParen {
              return Err(VMError(Box::new(ParseError::WronglyDottedList), info.clone()));
            }
          }
        }
...
fn parse_rec<I: Iterator<Item = LexItem>>(
  ...
  let data = match lexemes.next() {
    ...
    Some(LexItem(Lexeme::Symbol(x), info))
      => DataCell::new_info(x, info),
    ...
    Some(LexItem(Lexeme::LParen, info))
      => parse_list(lexemes, symbols, info)?,
...
mod tests {
  ...
  macro_rules! parse_str {
    ($vm:expr, $s:expr) => {{
      ...
      results.into_iter().map(|r| r.data.clone().into_inner()).collect::<Vec<_>>()
    }}
  }
  ...
  fn err_mismatched_l_paren() {
    ...
    assert_eq!(ParseError::MismatchedLParen, *result.0.downcast::<ParseError>().unwrap());
  }
  ...
  fn err_mismatched_r_paren() {
    ...
    let lexemes = vm.lex("(a 'b '()'c d))".chars()).unwrap();
    let result = vm.build_ast(lexemes).unwrap_err();
    assert_eq!(ParseError::MismatchedRParen, *result.0.downcast::<ParseError>().unwrap());
  }
  ...
  fn err_quoted_r_paren() {
    ...
    let lexemes = vm.lex("(a 'b (')'c d)".chars()).unwrap();
    let result = vm.build_ast(lexemes).unwrap_err();
    assert_eq!(ParseError::QuotedRParen, *result.0.downcast::<ParseError>().unwrap());
  }
  ...
  fn err_wrongly_dotted_list() {
    ...
    let result = vm.build_ast(lexemes).unwrap_err();
    assert_eq!(ParseError::WronglyDottedList, *result.0.downcast::<ParseError>().unwrap());
  }
}
